...
export default function AgendarCita() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get("/medicos"),
      api.get("/catalogos/consultas"),
      api.get("/catalogos/eventos"),
    ])
      .then(([e, m, c, v]) =>
        setCats({
      ...
      .catch((err) =>
        console.error("[AgendarCita] Carga de catálogos falló", err)
      );
  }, []);
  ...
  async function submit(e) {
    ...
    try {
      ...
      setMsg(err?.response?.data?.error || "Error al crear cita");
    }
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={submit}>
        ...
        <select
          value={f.consulta_id}
          onChange={(e) => up("consulta_id", e.target.value)}
        >
          <option value="">Consulta</option>
          {cats.consultas.map((c) => (
        ...
        <select
          value={f.evento_id}
          onChange={(e) => up("evento_id", e.target.value)}
        >
